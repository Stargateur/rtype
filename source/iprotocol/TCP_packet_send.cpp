...
// Last update Tue Dec  8 18:18:17 2015 Antoine Plaskowski
//
...
void	TCP_packet_send::put_string(std::string const &string)
{
  put_int<uint8_t>(static_cast<uint8_t>(string.size()));
  for (uintmax_t i = 0; i < string.size(); i++)
    put_int<uint8_t>(static_cast<uint8_t>(string[i]));
}
